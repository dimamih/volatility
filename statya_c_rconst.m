...
t1 = T - t;
g = interp2(x,t,trueu,phi,t1); %реальная цена опциона акции
...
figure; %графики цены акции и опциона
subplot(2,1,1);
...
for i = 1:N
    ...
    J(i) = t(2)*trapz((g_i-g).^2); %значение функционала
...
%     mu = 0.2;
    ...
    alpha0 = 1;
    u_pr = u_c(x,t,vsig(i,:)+alpha0*a_grad,r*ones(1,size(x,2)),u0,K);
    J_pr = t(2)*trapz((interp2(x,t,u_pr,phi,t1)-g).^2);
    J0 = J_pr;
    ii = 0;
    while (alpha0*dot_pr*0.2 > J(i) - J_pr)
        disp(['ii=',num2str(ii),', alpha0*dot_pr*0.2=',num2str(alpha0*dot_pr/2),...
            ', J(i) - J_pr=',num2str(J(i) - J_pr)]);
        alpha0 = alpha0/2;
        u_pr = u_c(x,t,vsig(i,:)+alpha0*a_grad,r*ones(1,size(x,2)),u0,K);
        J_pr = t(2)*trapz((interp2(x,t,u_pr,phi,t1)-g).^2);
        ii = ii + 1;
...
g_i = interp2(x,t,u,phi,t1);
J(N+1) = t(2)*trapz((g_i-g).^2); %значение функционала
